...
IdiosyncDict={
    ...
    "LivPrb" : [.978],                   # Survival probability
    ...
    #New Economy Parameters
     "SSWmu " : 1.015 ,                      # Wage Markup from sequence space jacobian appendix
...
plt.plot((C_dx0[1:]- C_dx0[1:])/(jac_agent.dx), label = '0')
plt.plot((CHist[4][1:]- C_dx0[1:])/(jac_agent.dx), label = '40')
plt.plot((CHist[3][1:]- C_dx0[1:])/(jac_agent.dx), label = '15')
plt.plot((CHist[1][1:]- C_dx0[1:])/(jac_agent.dx), label = '1')
plt.plot((CHist[2][1:] - C_dx0[1:])/(jac_agent.dx), label = '5')
plt.ylim([-.14,.01])
plt.legend()
plt.show()
