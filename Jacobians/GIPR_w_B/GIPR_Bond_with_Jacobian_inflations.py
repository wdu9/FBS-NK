...
#I think the reason why the graphs flip when we set wage stickiness to be 'stickier' than price stickiness is because if nominal wages fall less it may induce the real wage to flip direction and than inverting everything
...
# Note for monetary policy shocks markups move in the same direction , so both the nominal wage and nominal price wants to fall, if prices are more flexible or wages are stickier, than the real wage will actually rise inverting everything because the consumption effect is so strong.
...
#IF YOU HAVE NONZERO TAYLOR RULE COEFFICIENTS IT DOESNT HAVE THE PROBLEM ANYMORE but what then happens is that, 
#for a rise in the nominal rate, having a taylor rule coefficient pushes down on inflation so much, that it actually lowers the rate
# Also, if you set , stickiness params , both to .75, it will flip, but if you raise v = 5, and more signficantly, v=7 , the issue goes away
# for productivity shocks, to ensure the response is in the right direction and not too large, setting v =15 fixes the problem.
...
for i in range(199):
        pi_forward[i-1, i ] = 1/(1+rstar) # because r_{t} = r_{t+1}^{a}
...
for i in range(199):
        piw_forward[i-1, i ] = LivPrb*DiscFac # because r_{t} = r_{t+1}^{a}
...
h2z[:,0:200] = np.dot(J_pi_mup,J_mup_Z)
