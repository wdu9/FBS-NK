# -*- coding: utf-8 -*-
"""
Created on Wed Jun 23 20:04:06 2021
...
from Automatic_JAC import jac,simpleJAC, get_simpleJAC
...
args=(N_ss, w_ss, N_ss)
J_D = simpleJAC(dividends,args,T=200)
...
args = (Z_ss,N_ss)
J_Y = simpleJAC(production,a = args,T=200)
J_Y_Z = J_Y[0]  
J_Y_N = J_Y[1]
...
args =(0.0,N_ss,0.0)
J_i = simpleJAC(taylor,args,T=200)
...
J_pi =jac(price_inflation,args,T=200)
J_pi_w = J_pi[0]
J_pi_Z = J_pi[1]
...
args = (0.0,w_ss,Z_ss)
J_pi1 = simpleJAC(price_inflation_tom,args,T=200)
J_pi1_pi0 = J_pi1[0]
...
J_piw = jac(wageinflation,args,T=200)
...
args = (1.0,)
J_ra = simpleJAC(finance,args,T=200, minusone = [True])
J_ra_r = J_ra[0]
...
J_qs = jac(stock,args,T=200,plusone=[True,True,False])
J_qs_D = J_qs[0]
J_qs_r = J_qs[1]
...
J_B = jac(Bond,args,T=200,plusone =[True,True,True])
...
args= (rstar,)
J_qb = simpleJAC(BondPrice,args,T=200)
...
h1 = np.zeros((200,600))
h1[:,0:200] = np.dot(AJAC,J_ra_r) - J_qs_r - ( (J_B_r*(1+rstar) - B_ss) / (1+rstar)**2)  #Partials wrt r
h1[:,200:400] =  AJACW - np.dot(J_qs_D,J_D_w) - J_B_w/(1+rstar)  #partials wrt w
h1[:,400:600] = AJACN  - np.dot(J_qs_D,J_D_N) - J_B_N/(1+rstar) #Partials wrt N
...
h3[:, 200:400] = (1 + rstar) * J_pi1_w - phi * J_pi_w 
...
#h1z[:,0:200] = - np.dot(J_qs_D,J_D_Z)

h2z = np.zeros((200,400))
h2z[:,0:200] = J_pi_Z

h3z = np.zeros((200,400))
h3z[:,0:200] = (1+rstar)*J_pi1_Z - phi * J_pi_Z - phi_y * J_Y_Z
h3z[:,200:400] = - J_i_v
