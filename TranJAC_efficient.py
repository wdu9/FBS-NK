...
FBSDict={
    ...
    "DiscFac": 0.9713038536919956, #.96,                     # Intertemporal discount factor
...
for k in range(example.T_cycle): 
    ...
    #aNrmList.append(aNext)
    
    Cnow = example.solution[k].cFunc(Dist_mGrid)
    #CNrmList.append(Cnow)
    ...
    #TranMatList.append(TranMatrix)
    #print(len(TranMatList))
...
plt.legend()
plt.savefig("AJAC_TRANMAT.jpg", dpi=500)
plt.show()
...
plt.plot(np.zeros(len(AggA_List)), color= 'k')
plt.legend()
plt.savefig("CJAC_TRANMAT.jpg", dpi=500)
plt.show()
...
for i in range(T):
    ...
    Dstn = np.dot(TranMatList[i],Dstn)
